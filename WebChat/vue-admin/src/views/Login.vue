        ...
        <form>
          ...
          <div class="login_form_input_group">
            <div class="login_form_input_wraper">
              ...
              <input class="login_form_input" v-model="user.loginId" />
            </div>
          </div>
          <div class="login_form_input_group">
            <div class="login_form_input_wraper">
              <label class="login_form_input_label"> Password </label>
              <input class="login_form_input" type="password" v-model="user.password" />
            </div>
          </div>
        </form>
        <div class="login_form_btn_wrapper">
          <button class="login_form_btn" @click="signIn()" :disabled="throttle">
...
export default {
  ...
  methods: {
    async signIn() {
      this.throttle = true
      try {
        let res = await this.axios.post('/api/Admin/SignIn', this.user)
        window.console.log(res)
        if (res.data.resultCode !== '10') throw res.data.error

        this.$store.state.system.token = res.data.token
        localStorage.setItem('token', res.data.token)
        let payload = jwt.parseJwt(res.data.token)
        this.$store.state.system.userId = parseInt(payload.id)
        this.$store.state.system.loginId = payload.loginId
        this.$store.state.system.userName = payload.userName
        this.$store.state.system.photo = payload.photo
        // 檢查是不是管理員(後端已有機制，前端再行檢查)
        let isAdmin = Array.isArray(payload.roles)
          ? payload.roles.includes('Admin')
          : payload.roles === 'Admin'
        if (isAdmin) {
          this.$store.state.system.isLogin = true
